    ...
    for record in records:
        ...
        cursor.execute("""
            INSERT INTO records (
                ...
                resource_type, medium, reading_score, reading_education, 
                reading_description, language_code, language_encoding, credits
            ) VALUES (
                %(title)s, %(date_of_creation)s, %(release_date)s, %(description)s, %(url)s,
                %(bibliographic_level)s, %(encoding_level)s, %(catalog_details)s,
                %(resource_type)s, %(medium)s, %(reading_score)s, %(reading_education)s, 
                %(reading_description)s, %(language_code)s, %(language_encoding)s, %(credits)s
            ) RETURNING id""", data)
